 ...
 *                  Many concurrent writers are safe. This is the durable data.
 *
...
const BOARD_START = '<!-- ALFRED-BOARD-START -->';
const BOARD_END = '<!-- ALFRED-BOARD-END -->';
const EMPTY = '_(empty — ALFRED will fill this on first curation)_';

/** Strip anything that could break our markers out of board content. */
function sanitizeBoard(s) {
  return String(s).replace(
    /<!--\s*ALFRED-BOARD-(START|END)\s*-->/gi,
    '[alfred-board-$1]'
  );
}
...
function boardTemplate() {
  return [
    '# 🦇 THE KHAZANAH',
    "### Live cross-session memory · ALFRED's board",
    '',
    '> ALFRED curates this board from the raw capture log (.sharil/raw.log).',
    ...
    BOARD_END,
    '',
    '*The Khazanah never forgets. — ALFRED*',
    '',
  ].join('\n');
}
...
/**
 * Pure append to raw.log. Atomic at the OS level → safe for unlimited
 * concurrent writers (the 3-chatbox case). No read, no markers, no contention.
 */
